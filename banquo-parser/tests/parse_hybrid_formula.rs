...
fn parse_hybrid_formula_always_p1() {
    ...
    let guard_on_off = Guard::from(Predicate::from([Term::Variable("x".into(), -1.0), Term::Constant(-10.0)]));
    let guard_off_on = Guard::from(Predicate::from([Term::Variable("x".into(), 1.0), Term::Constant(5.0)]));
    let automaton = Automaton::from([
        (Mode::On, Mode::Off, guard_on_off),
        (Mode::Off, Mode::On, guard_off_on),
    ]);
    ...
    let trace = Trace::from([
        (0.0, hybrid_state(Mode::On, 1.0)),
        (1.0, hybrid_state(Mode::On, 2.0)),
    ]);
