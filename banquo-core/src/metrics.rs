...
#[repr(transparent)]
struct Robustness(NotNan<f64>);

impl Robustness {
    /// Create a new Robustness value from a given double
    ///
    /// # Safety
    /// Behavior is undefined if value is NaN
    pub unsafe fn new(value: f64) -> Self {
        Self(NotNan::new_unchecked(value))
    }
}

impl AsRef<f64> for Robustness {
    fn as_ref(&self) -> &f64 {
        self.0.as_ref()
    }
}
...
#[derive(Debug, Error)]
#[error("Provided Robustness value is NaN")]
pub struct NanRobustnessValue {
    #[from]
    source: ordered_float::FloatIsNan,
}

impl TryFrom<f64> for Robustness {
    type Error = NanRobustnessValue;

    fn try_from(value: f64) -> Result<Self, Self::Error> {
        NotNan::try_from(value)
            .map(Self)
            .map_err(NanRobustnessValue::from)
    }
}

impl From<Robustness> for f64 {
    fn from(value: Robustness) -> Self {
        value.0.into_inner()
    }
}

impl PartialEq<f64> for Robustness {
    fn eq(&self, other: &f64) -> bool {
        self.0.eq(other)
    }
}

impl PartialOrd<f64> for Robustness {
    fn partial_cmp(&self, other: &f64) -> Option<std::cmp::Ordering> {
        self.0.as_ref().partial_cmp(other)
    }
}

impl Neg for Robustness {
    type Output = Self;
...
impl Top for Robustness {
    fn top() -> Self {
        unsafe {
            Self(NotNan::new_unchecked(f64::INFINITY))
        }
    }
}

impl Bottom for Robustness {
    fn bottom() -> Self {
        unsafe {
            Self(NotNan::new_unchecked(f64::NEG_INFINITY))
        }
    }
}

impl Meet for Robustness {
    fn min(&self, other: &Self) -> Self {
        Self(NotNan::min(self.0, other.0))
    }
}

impl Join for Robustness {
    fn max(&self, other: &Self) -> Self {
        Self(NotNan::max(self.0, other.0))
    }
}
